    ...
    try:
        ...
        if 'primeira vez' in resposta['status']:
            ...
            for i in range(fila['qtd_fila']):
                campo_de_texto.send_keys(
                    f"{i+1} - *{fila['nome'][i]}* ({fila['telefone'][i]})", Keys.SHIFT + Keys.ENTER)
            ...
            campo_de_texto.send_keys(
                "_(Se você for o primeiro a ser atendido no dia, você ganha 2,00 de desconto)_", Keys.SHIFT + Keys.ENTER)
            paste_content(driver, campo_de_texto, '💵')
            ...
            paste_content(driver, campo_de_texto, '⌛')
            campo_de_texto.send_keys(
                " Promoções válidas *APENAS* para agendamentos marcados pelo Whatsapp até o final desta semana", Keys.SHIFT + Keys.ENTER)
            campo_de_texto.send_keys(Keys.SHIFT + Keys.ENTER)
            ...
            if 'info' in resposta['frase']:
                ...
                campo_de_texto.send_keys(Keys.SHIFT + Keys.ENTER)
                campo_de_texto.send_keys(
                    "Para garantirmos um atendimento eficiente, solicitamos que chegue com *10 minutos* de antecedência ao salão. Além disso, concederemos uma tolerância de mais *5 minutos* após o horário previsto.", Keys.SHIFT + Keys.ENTER)
                campo_de_texto.send_keys(Keys.ENTER)
            ...
            campo_de_texto.send_keys(Keys.SHIFT + Keys.ENTER)
            paste_content(driver, campo_de_texto, '1️⃣')
            campo_de_texto.send_keys(" - *Sim*", Keys.ENTER)
            paste_content(driver, campo_de_texto, '2️⃣')
            campo_de_texto.send_keys(" - *Não*", Keys.ENTER)
        ...
        # ===== SUPER USER ===== #
            ...
            for i in range(len(clientes)):
                campo_de_texto.send_keys(
                    f"{i+1} - *{clientes[i]['nome']}* ({clientes[i]['telefone']}) - status: {clientes[i]['status']}", Keys.SHIFT + Keys.ENTER)
            campo_de_texto.send_keys(Keys.ENTER)
